...
create_cfg() {
  ...
  if [ -n "$SALOME_MED_ROOT" ]; then
    ...
    # Enable MED option and preprocessor flag for HERMES to load MED support
    MED_OPTIONS=" med"
    MED_FFLAGS=" -DHAVE_MED"
...
if [ -n "${_MED_ROOT}" ]; then
  ...
  # SALOME MED library depends on SALOME's bundled HDF5 (version-specific)
  # We must use SALOME's HDF5 at runtime, not the system one
  _SALOME_HDF5_ROOT="$(_first_dir \
    "${SALOME_ROOT}/BINARIES-DB12/hdf5" \
    "${SALOME_ROOT}/hdf5")"
  if [ -n "${_SALOME_HDF5_ROOT}" ]; then
    export _SALOME_HDF5_LIB="${_SALOME_HDF5_ROOT}/lib"
  fi
fi
...
# LD_LIBRARY_PATH
# Note: SALOME's HDF5 must come BEFORE system HDF5 if MED support is enabled,
# because SALOME's MED library was compiled against SALOME's specific HDF5 version.
for _libdir in \
  "${_SALOME_HDF5_LIB:-}" \
  "${_MPI_LIB}" \
  "${_HDF5_LIB}" \
...
if [ -n "${_MED_ROOT:-}" ]; then
  ...
  [ -n "${_SALOME_HDF5_LIB:-}" ] && echo "SALOME HDF5='${_SALOME_HDF5_LIB}' (used for MED compatibility)"
fi
